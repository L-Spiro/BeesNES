 ...
 *
 * Description: A Direct3D 12 command list.
 */


#ifdef LSN_DX12

#include "LSNDirectX12CommandList.h"

namespace lsn {

	// == Various constructors.
	CDirectX12CommandList::CDirectX12CommandList() {
	}
	CDirectX12CommandList::~CDirectX12CommandList() {
		Reset();
	}
	 ...
	 **/
	void CDirectX12CommandList::Reset() {
		if ( m_pclList ) {
			m_pclList->Release();
			m_pclList = nullptr;
		}
	 ...
	 **/
	bool CDirectX12CommandList::CreateCommandList(
		...
		Reset();
		return SUCCEEDED( _pd12dDevice->CreateCommandList( _uiNodeMask, _cltType, _pcaCommandAllocator, _psInitialState, IID_PPV_ARGS( &m_pclList ) ) );
	}

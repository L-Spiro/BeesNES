...
namespace lsn {
	...
	uint8_t * CDx9NtscLSpiroFilter::ApplyFilter( uint8_t * _pui8Input, uint32_t &_ui32Width, uint32_t &_ui32Height, uint16_t &/*_ui16BitDepth*/, uint32_t &_ui32Stride, uint64_t /*_ui64PpuFrame*/, uint64_t _ui64RenderStartCycle,
		...
		m_tuUploader.UploadTexels( &Device(), m_vRgbBuffer.data(), m_ui16ScaledWidth, m_ui32SrcH, ui32Pitch, D3DFMT_A32B32G32R32F );

		Render( rRect );

		_ui32Width = uint32_t( rRect.Width() );
		_ui32Height = uint32_t( rRect.Height() );
	...
	bool CDx9NtscLSpiroFilter::Render( const lsw::LSW_RECT &_rOutput ) {
		...
		return RenderBase( &Device(), m_tuUploader.GetTexture()->Get(), m_ui16ScaledWidth, m_ui32SrcH, _rOutput, false );
	}

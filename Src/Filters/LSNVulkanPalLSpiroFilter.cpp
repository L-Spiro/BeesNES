...
#include "LSNVulkanPalLSpiroFilter.h"
...
namespace lsn {
	 ...
	 */
	CDisplayClient::LSN_PPU_OUT_FORMAT CVulkanPalLSpiroFilter::Init( size_t _stBuffers, uint16_t _ui16Width, uint16_t _ui16Height ) {
		StopThreads();
	 ...
	 */
	uint8_t * CVulkanPalLSpiroFilter::ApplyFilter( uint8_t * _pui8Input, uint32_t &_ui32Width, uint32_t &_ui32Height, uint16_t &/*_ui16BitDepth*/, uint32_t &_ui32Stride, uint64_t /*_ui64PpuFrame*/, uint64_t _ui64RenderStartCycle,
	 ...
	 */
	void CVulkanPalLSpiroFilter::Activate() {
		CParent::Activate();
	 ...
	 */
	void CVulkanPalLSpiroFilter::DeActivate() {
	 ...
	 **/
	void CVulkanPalLSpiroFilter::FrameResize() {
	 ...
	 */
	void CVulkanPalLSpiroFilter::SetWorkerThreadCount( size_t _stThreads ) {
	 ...
	 **/
	void CVulkanPalLSpiroFilter::FilterFrame( const uint8_t * _pui8Pixels, uint64_t _ui64RenderStartCycle ) {
	 ...
	 */
	bool CVulkanPalLSpiroFilter::EnsureSizeAndResources() {
		m_bValidState = false;
	 ...
	 */
	void CVulkanPalLSpiroFilter::ReleaseSizeDependents() {
	 ...
	 * \return Returns true on success.
	 */
	bool CVulkanPalLSpiroFilter::UploadTexture() {
	 ...
	 */
	void CVulkanPalLSpiroFilter::StartThreads() {
		...
		if ( stWorkers ) {
			...
			for ( size_t I = 0; I < stWorkers; ++I ) {
				m_vThreads.emplace_back( &CVulkanPalLSpiroFilter::WorkerThread, this, I + 1 );
			}
		}
	 ...
	 */
	void CVulkanPalLSpiroFilter::StopThreads() {
		if ( !m_bThreadsStarted ) { return; }
	 ...
	 */
	void CVulkanPalLSpiroFilter::WorkerThread( size_t _stThreadIdx ) {

...
#include "LSNDx12NtscCrtFullFilter.h"
...
namespace lsn {
	...
	// == Members.
	CDx12NtscCrtFullFilter::CDx12NtscCrtFullFilter() :
		m_ui32FinalStride( 0 ) {
	 ...
	 */
	CDisplayClient::LSN_PPU_OUT_FORMAT CDx12NtscCrtFullFilter::Init( size_t _stBuffers, uint16_t _ui16Width, uint16_t _ui16Height ) {
	 ...
	 */
	uint8_t * CDx12NtscCrtFullFilter::ApplyFilter( uint8_t * _pui8Input, uint32_t &_ui32Width, uint32_t &_ui32Height, uint16_t &/*_ui16BitDepth*/, uint32_t &_ui32Stride, uint64_t /*_ui64PpuFrame*/, uint64_t _ui64RenderStartCycle,
	 ...
	 */
	void CDx12NtscCrtFullFilter::Activate() {
		CParent::Activate();
	 ...
	 */
	void CDx12NtscCrtFullFilter::DeActivate() {
		CParent::DeActivate();
	 ...
	 **/
	void CDx12NtscCrtFullFilter::FrameResize() {
		OnSizeDx12();
		EnsureSizeAndResources();
	}
	 ...
	 */
	bool CDx12NtscCrtFullFilter::EnsureSizeAndResources() {
		m_bValidState = false;
		if ( !m_pdx12dDevice ) {
	 ...
	 */
	void CDx12NtscCrtFullFilter::ReleaseSizeDependents() {
	 ...
	 */
	bool CDx12NtscCrtFullFilter::Render( const lsw::LSW_RECT &_rOutput ) {
		if LSN_UNLIKELY( !m_bValidState || !m_tuUploader.GetTexture() || !m_tuUploader.GetTexture()->Get() ) { return false; }

		if ( !m_tpsScaler.Render( m_pdx12dDevice, m_gclCommandList.get(), m_tuUploader.GetTexture(), m_ui32FinalWidth, m_ui32FinalHeight, GetActualHorSharpness(), GetActualVertSharpness(), CNesPalette::LSN_G_CRT2, m_bUse16BitInitialTarget ) ) {
			return false;
		}

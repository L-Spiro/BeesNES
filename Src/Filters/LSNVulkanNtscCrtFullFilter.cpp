 ...
 *
 * Description: LMP88959’s implementation of an NTSC filter for Vulkan 1.0.
 */

#include "LSNVulkanNtscCrtFullFilter.h"
...
namespace lsn {
	...
	// == Members.
	CVulkanNtscCrtFullFilter::CVulkanNtscCrtFullFilter() :
		m_ui32FinalStride( 0 ) {
	 ...
	 */
	CDisplayClient::LSN_PPU_OUT_FORMAT CVulkanNtscCrtFullFilter::Init( size_t _stBuffers, uint16_t _ui16Width, uint16_t _ui16Height ) {
	 ...
	 */
	uint8_t * CVulkanNtscCrtFullFilter::ApplyFilter( uint8_t * _pui8Input, uint32_t &_ui32Width, uint32_t &_ui32Height, uint16_t &/*_ui16BitDepth*/, uint32_t &_ui32Stride, uint64_t /*_ui64PpuFrame*/, uint64_t _ui64RenderStartCycle,
	 ...
	 */
	void CVulkanNtscCrtFullFilter::Activate() {
		CParent::Activate();
	 ...
	 */
	void CVulkanNtscCrtFullFilter::DeActivate() {
	 ...
	 **/
	void CVulkanNtscCrtFullFilter::FrameResize() {
	 ...
	 */
	bool CVulkanNtscCrtFullFilter::EnsureSizeAndResources() {
		m_bValidState = false;
	 ...
	 */
	void CVulkanNtscCrtFullFilter::ReleaseSizeDependents() {
	 ...
	 * \return Returns true on success.
	 */
	bool CVulkanNtscCrtFullFilter::UploadTexture() {

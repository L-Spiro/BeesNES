...
namespace lsn {
	...
	void CNtscLSpiroFilter::SetGamma( float _fGamma ) {
		...
		for (size_t I = 0; I < LSN_SRGB_RES; ++I ) {
			double dVal = CUtilities::LinearTosRGB_Precise( CUtilities::CrtProperToLinear( (I / (uint32_t( LSN_SRGB_RES ) - 1.0)) ) ) * 255.0;
			...
			dVal = CUtilities::LinearTosRGB_Precise( CUtilities::CrtProperToLinear( (I / (uint32_t( LSN_SRGB_RES ) - 1.0)), 1.0, 0.0181 * 0.5 ) ) * 255.0;

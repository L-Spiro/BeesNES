...
namespace lsn {
	...
	public :
		...
		bool										Read( _tType &_tValue ) const {
			if LSN_LIKELY( Read( reinterpret_cast<uint8_t *>(&_tValue), sizeof( _tValue ) ) == sizeof( _tValue ) ) { return true; }
			return false;
		}
		...
		bool										Write( _tType _tValue ) {
			if LSN_LIKELY( Write( reinterpret_cast<const uint8_t *>(&_tValue), sizeof( _tValue ) ) == sizeof( _tValue ) ) { return true; }
			return false;
		}

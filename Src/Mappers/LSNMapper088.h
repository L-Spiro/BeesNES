 ...
 *
 * Description: Mapper 088 implementation.
 */
...
namespace lsn {
	...
	/**
	 * Class CMapper088
	 * \brief Mapper 088 implementation.
	 *
	 * Description: Mapper 088 implementation.
	 */
	class CMapper088 : public CMapperBase {
	public :
		CMapper088() {
		}
		virtual ~CMapper088() {
		...
		virtual void									ApplyMap( CCpuBus * _pbCpuBus, CPpuBus * _pbPpuBus ) {
			...
			for ( uint32_t I = 0x8000; I < 0xA000; ++I ) {
				if ( (I & 0xE001) == 0x8000 ) {
					_pbCpuBus->SetWriteFunc( uint16_t( I ), &CMapper088::SelectBank8000_9FFE, this, 0 );
				}
				else if ( (I & 0xE001) == 0x8001 ) {
					_pbCpuBus->SetWriteFunc( uint16_t( I ), &CMapper088::SelectBank8001_9FFF, this, 0 );
				}
			}
	...
	protected :
		...
		static void LSN_FASTCALL						SelectBank8000_9FFE( void * _pvParm0, uint16_t /*_ui16Parm1*/, uint8_t * /*_pui8Data*/, uint8_t _ui8Val ) {
			CMapper088 * pmThis = reinterpret_cast<CMapper088 *>(_pvParm0);

 ...
 *
 * Description: Mapper 229 implementation.
 */
...
namespace lsn {
	...
	/**
	 * Class CMapper229
	 * \brief Mapper 229 implementation.
	 *
	 * Description: NES Mapper 229 (31-in-1 multicart) implementation.
	 */
	class CMapper229 : public CMapperBase {
	public :
		CMapper229() {
		}
		virtual ~CMapper229() {
		...
		virtual void									ApplyMap( CCpuBus * _pbCpuBus, CPpuBus * _pbPpuBus ) {
			...
			for ( uint32_t I = 0x8000; I < 0x10000; ++I ) {
				_pbCpuBus->SetWriteFunc( uint16_t( I ), &CMapper229::SelectBank8000_FFFF, this, uint16_t( I ) );
			}
	...
	protected :
		// == Members.
		uint8_t											m_ui8Bank;									/**< The current PRG and CHR bank (5 bits). */
		uint8_t											m_ui8Mirror;								/**< The mirroring mode (1 bit). */
		...
		void inline										UpdateBanks() {
			// Mapper 229 maps the exact same 16 KiB PRG bank at both $8000 and $C000.
			SetPgmBank<0, PgmBankSize()>( m_ui8Bank );
			
			// Mapper 229 maps the exact same 16 KiB PRG bank at both $8000 and $C000.
			// However, if all M bits (bank bits) are 0, then the second page of PRG gets mapped to 1 instead of 0.
			SetPgmBank<1, PgmBankSize()>( m_ui8Bank == 0 ? 1 : m_ui8Bank );
			
			// CHR bank uses the exact same bank value.
			SetChrBank<0, ChrBankSize()>( m_ui8Bank );

			m_mmMirror = (m_ui8Mirror == 1) ? LSN_MM_HORIZONTAL : LSN_MM_VERTICAL;
		}
		...
		static void LSN_FASTCALL						SelectBank8000_FFFF( void * _pvParm0, uint16_t _ui16Parm1, uint8_t * /*_pui8Data*/, uint8_t /*_ui8Val*/ ) {
			CMapper229 * pmThis = reinterpret_cast<CMapper229 *>(_pvParm0);
			 ...
			 */
			pmThis->m_ui8Bank = _ui16Parm1 & 0x1F;
			pmThis->m_ui8Mirror = (_ui16Parm1 >> 5) & 0x01;

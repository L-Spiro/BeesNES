 ...
 *
 * Description: Mapper 144 implementation.
 */
...
namespace lsn {
	...
	/**
	 * Class CMapper144
	 * \brief Mapper 144 implementation.
	 *
	 * Description: Mapper 144 implementation.
	 */
	class CMapper144 : public CMapperBase {
	public :
		CMapper144() {
		}
		virtual ~CMapper144() {
		...
		virtual void									ApplyMap( CCpuBus * _pbCpuBus, CPpuBus * _pbPpuBus ) {
			...
			if ( m_prRom->vPrgRom.size() ) {
				for ( uint32_t I = 0x8000; I < 0x10000; ++I ) {
					_pbCpuBus->SetWriteFunc( uint16_t( I ), &CMapper144::SelectBank8000_FFFF, this, uint16_t( I - 0x8000 ) );
				}
			}
		}
	...
	protected :
		// == Members.
		bool											m_bConflicts = true;				/**< Whether to emulate bus conflicts or not. */
		...
		static void LSN_FASTCALL						SelectBank8000_FFFF( void * _pvParm0, uint16_t _ui16Parm1, uint8_t * /*_pui8Data*/, uint8_t _ui8Val ) {
			CMapper144 * pmThis = reinterpret_cast<CMapper144 *>(_pvParm0);
